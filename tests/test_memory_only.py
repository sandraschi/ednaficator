"""
Completely standalone test for the memory engine.
This file can be run directly without importing any other project files.
"""
...
class MemoryEngine:
    """
    Enhanced memory system with MCP integration.

    This is a simplified version for testing that doesn't depend on other modules.
    """

    def __init__(self, memory_path: str | Path, mcp_endpoint: str = None):
        """
    ...
    async def initialize(self) -> None:
        ...
        print(f"Memory Engine initialized with {len(self.conversations)} conversations")

    async def _initialize_mcp_connection(self) -> None:
        """Initialize connection to basic-memory MCP server."""
        print(f"Initializing MCP connection to {self.mcp_endpoint}")
    ...
    async def _load_conversations(self) -> None:
        ...
        self.conversations = [
            Interaction.from_dict(item) if isinstance(item, dict) else item for item in data
        ]
    ...
    async def _load_workflow_templates(self) -> None:
        ...
        print(f"Loaded {len(self.workflow_templates)} workflow templates")
    ...
    async def _load_austrian_knowledge(self) -> None:
        ...
        print("Loaded Austrian knowledge base")
    ...
    async def get_context(
        ...
        context = {
            ...
            "recent_interactions": [i.to_dict() for i in self.conversations[-lookback:]]
    ...
    async def get_workflow_template(self, name: str) -> list[dict[str, Any]] | None:
        """
    ...
    async def save_workflow_template(
        self, name: str, steps: list[dict[str, Any]], overwrite: bool = False
    ) -> bool:
        """
        ...
        if name in self.workflow_templates and not overwrite:
            print(f"Workflow '{name}' already exists and overwrite=False")
            return False
    ...
    async def _find_relevant_knowledge(self, text: str) -> dict[str, Any]:
        ...
        for category, items in self.austrian_knowledge.items():
            ...
            relevant_items = {
                k: v for k, v in items.items() if any(word in text_lower for word in k.split())
            }
    ...
    async def _save_conversations(self) -> bool:
        ...
        data = [i.to_dict() if hasattr(i, "to_dict") else i for i in self.conversations]
...
async def test_memory_engine():
    """Run tests for the memory engine."""
    ...
    try:
        ...
        engine = MemoryEngine(memory_path=test_dir)
        await engine.initialize()

        # Test 1: Store and retrieve interaction
        print("\nTest 1: Storing and retrieving interactions")
        interaction = await engine.store_interaction(
            user_input="Wie ist das Wetter in Wien?",
            response="Das Wetter in Wien ist sonnig bei 25°C.",
            intent={"category": "weather", "action": "query", "target": "current"},
            actions_taken=[
                {"service": "weather", "action": "get_current", "params": {"location": "Wien"}}
            ],
        )
        print(f"- Stored interaction: {interaction.user_input}")

        # Test 2: Update user preference
        print("\nTest 2: Updating user preferences")
        await engine.update_user_preference("temperature_unit", "celsius")
        print(f"- Updated temperature_unit to: {engine.user_preferences['temperature_unit']}")

        # Test 3: Save and retrieve workflow
        print("\nTest 3: Managing workflow templates")
        workflow_steps = [
            {"action": "wake_up", "time": "07:00"},
            {"action": "read_news", "source": "derstandard.at"},
        ]
        await engine.save_workflow_template("morning_routine", workflow_steps)
        print(f"- Saved workflow 'morning_routine' with {len(workflow_steps)} steps")

        # Test 4: Get context
        print("\nTest 4: Getting context")
        context = await engine.get_context("Was ist meine Morgenroutine?")
        print(f"- Retrieved context with {len(context['relevant_workflows'])} relevant workflows")

        # Test 5: Persistence
        print("\nTest 5: Testing persistence")
        # Create a new engine instance to test loading
        new_engine = MemoryEngine(memory_path=test_dir)
        await new_engine.initialize()
        print(f"- Loaded {len(new_engine.conversations)} conversations from disk")
        print(f"- Loaded {len(new_engine.workflow_templates)} workflow templates")
