"""
Tests for the enhanced memory engine.
"""
...
import pytest
from ednaficator.memory.engine_enhanced import Interaction, MemoryEngine
...
class TestContextRetrieval:
    ...
    async def test_get_context_with_workflows(self, memory_engine):
        ...
        await memory_engine.save_workflow_template(
            "morning_routine", [{"action": "wake_up"}, {"action": "read_news"}]
        )

        # Get context with workflow matching
        context = await memory_engine.get_context(
            current_input="Was ist meine Morgenroutine?", include_workflows=True
        )
...
class TestPreferenceHandling:
    ...
    async def test_update_preference(self, memory_engine):
        ...
        with open(MEMORY_PATH / "user_preferences.json") as f:
    ...
    async def test_learn_from_interaction(self, memory_engine):
        ...
        # Initial value
        assert memory_engine.user_preferences.get("morning_routine_time") == "07:00"
...
class TestWorkflowManagement:
    ...
    async def test_save_and_retrieve_workflow(self, memory_engine):
        ...
        result = await memory_engine.save_workflow_template("my_morning_routine", workflow_steps)
...
class TestAustrianKnowledge:
    ...
    async def test_find_relevant_knowledge(self, memory_engine):
        ...
        memory_engine.austrian_knowledge["shopping"] = {"geizhals.at": "Price comparison"}
...
if __name__ == "__main__":
    pytest.main(["-v", "test_memory_engine.py"])
