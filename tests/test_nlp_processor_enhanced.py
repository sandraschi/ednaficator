...
from ednaficator.nlp.processor_enhanced import NLPProcessor
...
async def test_llm_fallback(nlp_processor):
    ...
    with patch("aiohttp.ClientSession.post", new_callable=AsyncMock) as mock_post:
        ...
        intent = await nlp_processor.parse_intent("Wo finde ich den besten Apfelstrudel in Wien?")
...
async def test_llm_error_handling(nlp_processor):
    ...
    with patch("aiohttp.ClientSession.post", side_effect=Exception("Connection error")):
        # Test a complex query that would trigger LLM fallback
        intent = await nlp_processor.parse_intent("Erzähl mir etwas über die Geschichte von Wien")
...
def test_entity_extraction(nlp_processor):
    ...
    test_cases = [
        ("Wie wird das Wetter in Graz?", {"location": [r"in\s+(\w+)"]}, {"location": "Graz"}),
        (
