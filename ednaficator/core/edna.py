...
class EdnaCore:
    def __init__(self, config: dict[str, Any], concierge_router: Any | None = None):
        ...
        self.concierge_router = concierge_router
    ...
    async def initialize(self) -> None:
        ...
        if await self.llm.is_available():
            ...
            logger.info(
                f"{self.llm.provider} models: {models[:8]}{'...' if len(models) > 8 else ''}"
            )
        ...
        if self.config.get("mode") == "concierge":
            logger.info(
                "Concierge mode — MCP orchestrator registered but family chat uses adapters"
            )
    ...
    async def _process(self, user_input: str) -> EdnaResponse:
        if self.config.get("mode") == "concierge" and self.concierge_router is not None:
            response = await self.concierge_router.process(
                user_input,
                llm=self.llm,
                history=self._history[-20:],
                plex_client=self.config.get("plex_default_client"),
            )
    ...
    async def _store_memory(self, user_input: str, response: EdnaResponse) -> None:
        try:
            ...
            await self.mcp_orchestrator.call_tool(
                ...
                "write_note",
                {
                    "title": f"edna-interaction-{datetime.now(UTC).strftime('%Y%m%d-%H%M%S')}",
                    "content": (f"User: {user_input}\nEdna: {response.message}\nTool: {tool_tag}"),
    ...
    def get_status(self) -> dict[str, Any]:
        return {
            ...
            "mode": self.config.get("mode", "orchestrator"),
        }
